...
## Default flow: HASI dashboard intake

By default, the intake runs inside the **HASI dashboard** (the Next.js UI in `autoresearch-conductor/hasi-ui/`). The conductor's REST API exposes the same file-bus this skill listens to — you don't launch any UI yourself; the conductor server is already up, and the dashboard's "New project" panel writes `intake.json`/`answers.json`/`change_request.json`/`confirm.json` for you to react to.

Orchestrate it like this:

1. **Wait for the form.** Use the `Monitor` tool with an until-loop to block until the user submits: `until [ -f "$WORKDIR/intake.json" ]; do sleep 2; done`. Read `intake.json` → `{idea, name?, domain?, code?, code_path?}`.
2. **Think (the brain is unchanged).** Run the same inference as Step 2 below (name/slug, ML-vs-other → executor name, measure + direction + signed/scientific regex, data, baseline, deps, budget). Use the form's `domain` hint if given. For code: if the form supplied `code` or `code_path`, that is the user's existing code (no cwd scan needed) — preserve it per Step 4.
3. **Ask follow-ups in the UI (only if needed).** For any code-detection confirmation or the one allowed critical-gap question: write `{"intro": "...", "questions":[{"id","text","hint"}]}` to `$WORKDIR/questions.json`, set `$WORKDIR/state.json` to `{"phase":"questions"}`, then `Monitor` until answers arrive: `until [ -f "$WORKDIR/answers.json" ]; do sleep 2; done`. Read it, then **delete `answers.json`** before any next round. Keep rounds minimal.
4. **Render variation 1 for review.** Fill `spec.template.md` (Step 4) into `$WORKDIR/spec_1.md`. Write `$WORKDIR/variations.json` = `{"selected":1,"variations":[{"id":1,"label":"<short label>","note":"<one-line what's distinctive>"}]}`. Set `$WORKDIR/state.json` to `{"phase":"review"}`. The UI shows it with the Assumptions block visible. **Do not** write the project's `spec.md` yet — that happens on confirm.
5. **Review loop — change requests build variations.** `Monitor` until either file appears: `until [ -f "$WORKDIR/change_request.json" ] || [ -f "$WORKDIR/confirm.json" ]; do sleep 2; done`, then:
   ...
   - **`confirm.json`** = `{id}` → that's the chosen variation. Go to step 6.
6. **Confirm + materialize.** Copy `$WORKDIR/spec_<id>.md` to `<slug>/.autoresearch/spec.md`. If the user had code, preserve it and write the executor skeleton now (Step 4's code rules), based on the chosen variation. Set `state.json` `{"phase":"confirmed"}`. The UI closes itself.
7. **Summary + next step (terminal).** Print a short recap of the **chosen** variation (name, what we optimize, measure + direction, data, budget) and: "Saved as the project spec. Next: run `/autoresearch-generate`." If several variations were made, mention which one was confirmed.

**Fallback to terminal chat** if the HASI dashboard isn't available: say so and run the chat flow below instead. The terminal flow is also fine if the user explicitly prefers it.
...
- **HASI dashboard is the default intake.** The Next.js UI in `autoresearch-conductor/hasi-ui/` drives the intake via the conductor's file-bus (`intake.json` / `questions.json` / `answers.json` / `change_request.json` / `confirm.json`); you don't launch any UI yourself. Fall back to terminal chat only if the dashboard isn't available or the user prefers it. The UI is just a front-end — the inference, template, and rules below are identical in both modes.
