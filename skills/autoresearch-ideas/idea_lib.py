...
basket.py (the CLI materializer) imports from here so the idea.md format, the
status lifecycle, the source classification, and the git-untracked contract are
defined once and never drift.
...
def delete_upload(root, filename):
    """Delete a document from idea_basket/ and drop its status entry.
    Returns True if removed, False if it wasn't there. Rejects path-escape."""
    base = os.path.basename((filename or "").strip())
    if not base or base.startswith(".") or not base.lower().endswith(DOC_EXTS):
        raise ValueError("invalid filename")
    target = os.path.join(root, BASKET_DIR, base)
