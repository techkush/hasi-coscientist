...
- The **basket panel** in the HASI dashboard (`autoresearch-conductor/hasi-ui/`) is the user-facing surface; this skill speaks to it through the conductor's file-bus (see below).
...
## Default flow: HASI dashboard basket panel

By default, drive the basket through the **Ideas panel** in the HASI dashboard. It shows the spec goal, the live basket table (idea · status · source — papers show the **link**, documents show the **file name**), a sidebar of added files with a new/visited badge, an **Upload files** button, an **Execute** control with a **paper-limit** input (default 10), a **New idea** button, and a **view idea.md** button.

Protocol — control files in `$WORKDIR`; `idea.md`/`idea_basket/` in the project root. `state.json` (you write) = `{phase, name, goal, paper_limit, message}` (phase: `idle|collecting|thinking|ready`). The conductor handles **uploads** straight to `idea_basket/`; it writes `execute.json {paper_limit}`, `propose.json {text}`, and `confirm.json {accept, force}` on the buttons. You write `proposal.json` for the review step.

Orchestrate it like this:

1. **Prepare.** Resolve `<root>` (**must have a spec** — else stop, send them to `/autoresearch-init`). Run `python <skill_dir>/basket.py init <root>`.
2. **Seed state.** Write `$WORKDIR/state.json` with `phase:"idle"` and the `name`/`goal` from the spec.
3. **Monitor** until `$WORKDIR/execute.json` **or** `propose.json` appears (uploads need no action from you — the conductor saves them straight to `idea_basket/`).
...
**Fallback to terminal** if the HASI dashboard isn't available or the user prefers it: ask web-search permission (one green-marked question), ask the paper limit (default 10), collect with `basket.py` (< 10 ideas), and for a user idea do the same rewrite + alignment opinion in chat before adding.
