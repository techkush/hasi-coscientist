...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p in ("/", "/index.html"):
            return self._send(200,
                              "HASI UI lives at http://localhost:3000 — this port serves /api/* only.",
        ...
        if p == "/api/gen/chat_proposal":
            return self._send(200, _rj(slug, "gen_chat_proposal.json", {"pending": False}))
    ...
    def do_POST(self):
        ...
        if p == "/api/gen/chat":
            # Two-phase: agent first writes gen_chat_proposal.json with an
            # alignment verdict + summary. The UI shows a dialog, then POSTs
            # /api/gen/chat/confirm with accept/reject.
            try:
                os.remove(os.path.join(_dir(slug), "gen_chat_proposal.json"))
            except FileNotFoundError:
                pass
            _wj(slug, "gen_chat.json", {"text": (b.get("text") or "").strip()})
            return self._send(200, {"ok": True})
        if p == "/api/gen/chat/confirm":
            _wj(slug, "gen_chat_confirm.json", {
        ...
        if p == "/api/ideas/delete":
            if not I:
                return self._send(404, {"error": "ideas skill not available"})
            try:
                removed = I.delete_upload(root, (b.get("name") or "").strip())
            except ValueError as e:
                return self._send(400, {"error": str(e)})
            return self._send(200, {"ok": True, "removed": bool(removed)})
...
def main():
    ...
    ap.add_argument("--bind", default=os.environ.get("AUTORESEARCH_BIND", "127.0.0.1"),
                    help="bind address; use 0.0.0.0 inside Docker so port-forwarding to the host works")
    a = ap.parse_args()
    ...
    print(f"autoresearch conductor: http://{a.bind}:{a.port}  (workdir={WORKDIR}, workspace={WORKSPACE})", flush=True)
    try:
        srv.serve_forever()
